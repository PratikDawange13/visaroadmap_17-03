...
class GraphState(TypedDict):
    ...
    job_roles: str
    noc_codes: str
    high_demand_nocs: str  # New field for high-demand NOC recommendations
    crs_score: str
    roadmap: str
...
Your response MUST follow this EXACT format for the recommended pathways section:

Recommended Pathways: 
Option A: PNP(OINP):(HEALTH Draw) 
Option B: PNP(OINP):(TRADE Draw) 
Option C: EEP:(TRADE Draw)

Where:
- PNP stands for Provincial Nominee Program followed by province code in parentheses
- EEP stands for Express Entry Program
- Draw type MUST be in parentheses with format: (DRAW_TYPE Draw)
- The draw types MUST be formatted exactly as: (HEALTH Draw), (TRADE Draw), (STEM Draw), (Transport Draw), or (Agriculture/Agri-Food Draw)

For example, a valid output would be:
Recommended Pathways: 
Option A: PNP(OINP):(HEALTH Draw) 
Option B: PNP(BCPNP):(HEALTH Draw) 
Option C: EEP:(HEALTH Draw)

DO NOT include any other text, explanations, or formatting in the recommended pathways section.
...
# Add all nodes including the new additional recommendations node
...
workflow.add_node("recommend_pathways", recommend_pathways)  # New node
...
workflow.add_edge("calculate_crs_score", "recommend_pathways")  # New edge
workflow.add_edge("recommend_pathways", "generate_roadmap")  # New edge
